class GEMLearning(torch.nn.Module):
    ...
    def train(self, data_loader, task):
        self.cur_task = task
        running_loss = 0.0
        input_stack = torch.zeros((traindata_len, 28*28))
        label_stack = torch.zeros((traindata_len))
        if cuda_available:
        ...
        for i, data in enumerate(data_loader):
            ...
            input_stack[i*batch_size: (i+1)*batch_size] = x.detach().clone()
            label_stack[i*batch_size: (i+1)*batch_size] = y.detach().clone()
                ...
                for k in range(0, self.cur_task):
                    ...
                    for params in self.parameters():
                        if params is not None:
                            ...
                            self.G[stpt:endpt, k].data.copy_(params.grad.detach().view(self.G[stpt:endpt, k].data.size()))
                            j += 1
            ...
            if self.cur_task > 0:
                ...
                grad = torch.cat(grad)
                self.G[:, self.cur_task].data.copy_(grad.detach())
                
                # Solve Quadratic Problem 
                prod = torch.mm(self.G[:, self.cur_task].unsqueeze(0), self.G[:, :self.cur_task])
                if(prod < 0).sum() != 0: # There are some violations: projection
        ...
        perm = torch.randperm(traindata_len)
